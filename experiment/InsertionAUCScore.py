...
def _load_params(cfg_path, data_path, top_traits):
    ...
    params.output_dir = getattr(params, "output_dir", "visualization/insertion_auc")
...
def _prepare_dataloader(data_root, batch_size):
    ...
    annotation_path = os.path.join("data", "annotations", "cub", "insertion_auc_full.txt")
...
def _insertion_curve(model, original, blurred, heatmap, target_idx, steps):
    ...
    with torch.no_grad():
        for frac in fractions:
            ...
            if k == 0:
                ...
                x_k = blurred * (1.0 - mask) + original * mask
            logits, _ = model(x_k)
...
def compute_insertion_auc(args):
    ...
    for idx, (images, targets) in enumerate(tqdm(loader, desc="Insertion AUC")):
        ...
        # Use raw attention without pruning (classif_head is untrained, so skip pruning)
        ...
        fractions, probs = _insertion_curve(model, images, blurred, heatmap, target_cls, args.steps)
        ...
        if args.plot_dir:
            try:
                ...
                plt.plot(fractions, probs, marker='o', linewidth=2, markersize=4)
                plt.fill_between(fractions, probs, alpha=0.2)
                plt.xlabel('Fraction of Pixels Inserted')
                plt.ylabel('P(ground-truth class)')
                plt.title(f'Insertion Curve (AUC={auc:.3f})\n{os.path.basename(image_rel_path)}')
    ...
    print(f"Mean Insertion AUC: {mean_auc:.4f}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Insertion AUC Score for CUB")
    ...
    parser.add_argument("--output-csv", type=str, default="output/insertion_auc_scores.csv", help="Where to save per-image AUC values")
    parser.add_argument("--steps", type=int, default=100, help="Number of insertion steps")
    parser.add_argument("--top-traits", type=int, default=4, help="Number of heads retained by prune_attention_heads")
    parser.add_argument("--plot-dir", type=str, default="", help="Directory to save per-image insertion curves (PNG)")
    return parser.parse_args()


if __name__ == "__main__":
    args = parse_args()
    compute_insertion_auc(args)
